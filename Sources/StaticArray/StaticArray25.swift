...
public struct StaticArray25<T> {
    public typealias Values = (T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T)
    ...
    @inlinable public func map<M>(_ transform: (T) throws -> M) rethrows -> StaticArray25<M> {
        try StaticArray25<M>(values: (
            transform(self[0]),
            transform(self[1]),
            transform(self[2]),
            transform(self[3]),
...
public func zip<L, R>(_ lhs: StaticArray25<L>, _ rhs: StaticArray25<R>) -> StaticArray25<(L, R)> {
    StaticArray25(values: (
        (lhs[0], rhs[0]),
        (lhs[1], rhs[1]),
        (lhs[2], rhs[2]),
        (lhs[3], rhs[3]),
...
extension StaticArray25: Collection {
    public typealias Index = Int
    public var startIndex: Index { 0 }
    public var endIndex: Index { 25 }
    @inlinable public func index(after i: Index) -> Index { i + 1 }
}

extension StaticArray25: Sequence {
    public struct Iterator: IteratorProtocol {
        private var index = 0
        private let array: StaticArray25<T>

        init(_ array: StaticArray25<T>) {
            self.array = array
        }
...
extension StaticArray25: Equatable where T: Equatable {
    @inlinable public static func == (lhs: Self, rhs: Self) -> Bool {
        zip(lhs, rhs).allSatisfy { $0.0 == $0.1 }
    }
}

extension StaticArray25: Codable where T: Codable {
...
extension StaticArray25: AdditiveArithmetic where T: AdditiveArithmetic {
...
@inlinable public func + <T>(lhs: StaticArray25<T>, rhs: T) -> StaticArray25<T> where T: AdditiveArithmetic {
    lhs.map { $0 + rhs }
}

@inlinable public func + <T>(lhs: T, rhs: StaticArray25<T>) -> StaticArray25<T> where T: AdditiveArithmetic {
    rhs.map { lhs + $0 }
}
...
extension StaticArray25: Divisible where T: Divisible {
    @inlinable public static func / (lhs: Self, rhs: Self) -> Self {
        zip(lhs, rhs).map { $0.0 / $0.1 }
    }
}

@inlinable public func / <T>(lhs: StaticArray25<T>, rhs: T) -> StaticArray25<T> where T: Divisible {
    lhs.map { $0 / rhs }
}

@inlinable public func / <T>(lhs: T, rhs: StaticArray25<T>) -> StaticArray25<T> where T: Divisible {
    rhs.map { lhs / $0 }
}

extension StaticArray25: Multipliable where T: Multipliable {
    @inlinable public static func * (lhs: Self, rhs: Self) -> Self {
        zip(lhs, rhs).map { $0.0 * $0.1 }
    }
}

@inlinable public func * <T>(lhs: StaticArray25<T>, rhs: T) -> StaticArray25<T> where T: Multipliable {
    lhs.map { $0 * rhs }
}

@inlinable public func * <T>(lhs: T, rhs: StaticArray25<T>) -> StaticArray25<T> where T: Multipliable {
    rhs.map { lhs * $0 }
}

extension StaticArray25: ABSComputable where T: ABSComputable {
