...
public struct StaticArray26<T> {
    public typealias Values = (T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T)
    ...
    init(array: [T]) throws {
        guard array.count == 26 else { fatalError("array size mismatch") }
        self.values = (
            array[0],
            array[1],
            array[2],
    ...
    @inlinable public func map<M>(_ transform: (T) throws -> M) rethrows -> StaticArray26<M> {
        try StaticArray26<M>(values: (
            transform(self[0]),
            transform(self[1]),
            transform(self[2]),
            transform(self[3]),
...
public func zip<L, R>(_ lhs: StaticArray26<L>, _ rhs: StaticArray26<R>) -> StaticArray26<(L, R)> {
    StaticArray26(values: (
        (lhs[0], rhs[0]),
        (lhs[1], rhs[1]),
        (lhs[2], rhs[2]),
        (lhs[3], rhs[3]),
...
extension StaticArray26: Collection {
    public typealias Index = Int
    public var startIndex: Index { 0 }
    public var endIndex: Index { 26 }
    @inlinable public func index(after i: Index) -> Index { i + 1 }
}

extension StaticArray26: Sequence {
    public struct Iterator: IteratorProtocol {
        private var index = 0
        private let array: StaticArray26<T>

        init(_ array: StaticArray26<T>) {
            self.array = array
        }
...
extension StaticArray26: Equatable where T: Equatable {
    @inlinable public static func == (lhs: Self, rhs: Self) -> Bool {
        zip(lhs, rhs).allSatisfy { $0.0 == $0.1 }
    }
}

extension StaticArray26: Codable where T: Codable {
...
extension StaticArray26: AdditiveArithmetic where T: AdditiveArithmetic {
...
@inlinable public func + <T>(lhs: StaticArray26<T>, rhs: T) -> StaticArray26<T> where T: AdditiveArithmetic {
    lhs.map { $0 + rhs }
}

@inlinable public func + <T>(lhs: T, rhs: StaticArray26<T>) -> StaticArray26<T> where T: AdditiveArithmetic {
    rhs.map { lhs + $0 }
}
...
extension StaticArray26: Divisible where T: Divisible {
    @inlinable public static func / (lhs: Self, rhs: Self) -> Self {
        zip(lhs, rhs).map { $0.0 / $0.1 }
    }
}

@inlinable public func / <T>(lhs: StaticArray26<T>, rhs: T) -> StaticArray26<T> where T: Divisible {
    lhs.map { $0 / rhs }
}

@inlinable public func / <T>(lhs: T, rhs: StaticArray26<T>) -> StaticArray26<T> where T: Divisible {
    rhs.map { lhs / $0 }
}

extension StaticArray26: Multipliable where T: Multipliable {
    @inlinable public static func * (lhs: Self, rhs: Self) -> Self {
        zip(lhs, rhs).map { $0.0 * $0.1 }
    }
}

@inlinable public func * <T>(lhs: StaticArray26<T>, rhs: T) -> StaticArray26<T> where T: Multipliable {
    lhs.map { $0 * rhs }
}

@inlinable public func * <T>(lhs: T, rhs: StaticArray26<T>) -> StaticArray26<T> where T: Multipliable {
    rhs.map { lhs * $0 }
}

extension StaticArray26: ABSComputable where T: ABSComputable {
